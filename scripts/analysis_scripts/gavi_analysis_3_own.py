...
# single portrait figure spanning 2015–2024 (for vertical placement in LaTeX).
# Includes COVID shading (2020–2021) and Gavi MIC entry line (2022).
#
# Outputs:
#   - PDF (vector) always
...
def make_plot(
    ...
    # Standard orientation: Year on x-axis, coverage on y-axis
    ...
    # Plot each group (year on x, coverage on y)
    ...
    # Small in-plot note (lower right)
    ax.text(
        0.98, 0.02,
...
def main():
    parser = argparse.ArgumentParser(description="Generate Figure 3 (2015–2024, portrait).")
    parser.add_argument("--input", type=str,
                        default=str(Path(__file__).resolve().parent.parent.parent / "dataset_country_analysis_with_gavi_trajectory.xlsx"),
                        help="Path to dataset_country_analysis_with_gavi_trajectory.xlsx. Defaults to project-root file.")
    parser.add_argument("--outdir", type=str,
                        default=str(Path(__file__).resolve().parent.parent.parent / "fig"),
                        help="Output directory for figures and tables. Defaults to project-root fig/.")
    ...
    out_pdf = outdir / "Figure3_raw_vs_model_predicted_trajectories_NONHIC_2015_2024.pdf"
    out_png = outdir / "Figure3_raw_vs_model_predicted_trajectories_NONHIC_2015_2024.png"
    ...
    # ------------------------------------------------------------
    # Single portrait figure: 2015–2024
    # ------------------------------------------------------------
    make_plot(
        outpath_pdf=out_pdf,
        outpath_png=(out_png if args.png else None),
        raw_means=raw_means,
        pred_df=pred_df,
